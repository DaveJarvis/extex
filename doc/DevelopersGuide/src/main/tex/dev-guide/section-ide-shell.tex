...
installed and configured in some way\footnote{For instance if you are
  behind a firewall you might have to configure the proxy\index{proxy}
  settings in \texttt{servers.xml}\index{Subversion!servers.xml|tt}}.
...
\verb|JAVA_HOME|\index{JAVA\_HOME}\index{environment variable!JAVA\_HOME}
should be defined which points to the JDK. The following jars should
be placed on the environment variable
\verb|CLASSPATH|\index{CLASSPATH}\index{environment variable!CLASSPATH}: 
\begin{itemize}
