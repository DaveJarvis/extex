...
core. Currently the versions 3.4 (Ganymede\index{Eclipse!Ganymede})
and 3.5 (Galileo\index{Eclipse!Galileo}) of Eclipse is used within the
\ExTeX\ development team.
\begin{figure}[h]
...
\item [eclipse-jee-galileo-win32.zip]\ \\
  for any decent Windows\index{Windows} platform.
\item [eclipse-jee-galileo-linux-gtk.tar.gz]\ \\
  for Linux\index{Linux} on Intel x86 (32 bit) with GTK.
\end{description}

Note that we use and show here the jee edition\index{Eclipse!jee
  edition} of eclipse. Other editions will also work but show a
...
\begin{figure}[h]
  ...
  \caption{Eclipse Workspace}\index{Eclipse workspace}\label{fig:eclipse-workspace}
\end{figure}
When Eclipse starts you first see the splash screen shown in
figure~\ref{fig:eclipse}. Then Eclipse requests a workspace -- as
shown in figure~\ref{fig:eclipse-workspace}. The
...
\subsection{Installing Eclipse Plugins}\label{sec:eclipse.plugins}\index{Eclipse!Plugins|(}
...
\begin{tabular}{lll}
  ...
  Subversive\index{Eclipse!Plugins!Subversive} & ... & highly recommended \\
  Checkstyle\index{Eclipse!Plugins!Checkstyle} & ... & recommended \\
  FindBugs\index{Eclipse!Plugins!FindBugs}     & ... & recommended \\
  Eclemma\index{Eclipse!Plugins!Eclemma}       & ... & recommended \\
\end{tabular}
...
hosted by Berlios\index{Berlios}. Thus we start to get things onto the
local host. For this purpose we need to open a new perspective in
Eclipse. A perspective is a collection of windows which are usually
meant for a common task.

A new perspective can be opened via the window \menu{Window \sub Open
  Perspective \sub Other\ldots} which can be seen in
figure~\ref{fig:eclipse-open-perspective-other}. This menu item opens a
...
perspective is selected (see figure~\ref{fig:eclipse-select-svn}) and
the dialog is completed with the OK button.
\begin{figure}[ht]
  \hbox{}\hfill
  \subfigure[Open Perspective\label{fig:eclipse-open-perspective-other}]{%
    \includegraphics[scale=.4]{image/eclipse-open-perspective-other}}%
  \hfill
  \subfigure[Selecting ``Subversion Exploring
...
\begin{figure}[htp]
  ...
  \hfill
  \subfigure[The Coordinates of the \ExTeX\ Subversion
  Repository\label{fig:eclipse-add-svn}]{%
    \includegraphics[scale=.4]{image/eclipse-add-svn}}%
  \caption{Adding the \ExTeX\ CVS Repository}
\end{figure}

Note that you have to enter your account at Berlios\index{Berlios} and
its password into the appropriate fields. If you do not have an
account you can use the account name \texttt{anonymous} without any
password to get reading access to the sources.
