...
figure~\ref{fig:eclipse-workspace}. The workspace\index{Eclipse
  workspace} is a directory where the projects live and where your
preferences are stored. If you have chosen the workspace directory
carefully, you can turn on the check mark in this dialog to be not
asked again.
...
\begin{figure}[ht]
  \hbox{}\hfill
  \subfigure[Install New Software Menu\label{fig:eclipse-install-new-software}]{%
    \includegraphics[scale=.4]{image/eclipse/install-new-software}}%
  \hfill
  \subfigure[The Installation Dialog
...
\subsubsection{Checkstyle}\index{Eclipse!Plugins!Checkstyle}

Checkstyle\index{Checkstyle} is a tool for checking the adherence of
Java source code to certain rules. The rules can be freely configured.
The \ExTeX\ repository contains a set of rules for checkstyle.
\begin{description}
...
To enable the checks set in \menu{Project \sub Properties \sub
  Chekcstyle} the configuration to \emph{ExTeX} (see
...
This is an integration of the FindBugs\index{FindBugs} analyzer to
detect code anomalies in Eclipse.
\begin{description}
...
This is an integration of the code coverage tool Emma\index{Emma} into
Eclipse.
\begin{description}
...
figure~\ref{fig:eclipse-svn}.
\begin{figure}[htbp]
  \hbox{}\hfill
  \subfigure[The \ExTeX\ Repository Listed\label{fig:eclipse-svn}]{%
...
(see figure~\ref{fig:eclipse-svn}) and select \menu{Checkout} in the
