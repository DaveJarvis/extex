...
stored the environment variable \verb|$HOME|. On a Windows system the
...
intelligence test under Windows\index{Windows};-). It contains one
...
separator is a colon (\verb|:|) under Unix\index{Unix} and a semicolon
(\verb|;|) under Windows\index{Windows}.
...
On Windows TortoiseSNV\index{TortoiseSVN}
(\url{http://tortoisesvn.tigris.org/}) can be used. It is an
integration into the Explorer which simplifies the access to a
subversion repository without a separate client. \index{Subversion|)}


\section{Maven}\index{Maven|(}

The build system is based on Maven~2 (\url{http://maven.apache.org/}).
Thus Maven~2 needs to be installed. \index{Maven|)}
