 ...
 * 
 * <p>
 * The array contains instructions in a simple programming language that
 * explains what to do for special letter pairs. Each word is a lig_kern_command
 * of four bytes.
 * </p>
 * 
 ...
 * <tr>
 * <td>first </td>
 * <td>skip_byte, indicates that this is the program step if the byte is 128 or
 * more, otherwise the next step is obtained by skipping this number of
 * intervening steps.</td>
 * </tr>
 * <tr>
 * <td>second </td>
 * <td>next_char: if next_char follows the current character, then perform the
 * operation and stop, otherwise continue.</td>
 * </tr>
 * <tr>
 * <td>third </td>
 * <td>op_byte, indicates a ligature step if less than 128, a kern step
 * otherwise.</td>
 * </tr>
 * <tr>
 * <td>fourth </td>
 ...
 * 
 * <p>
 * There are eight kinds of ligature steps, having op_byte codes 4a+2b+c where 0
 * &lt; a &lt; b+c and 0 &lt; b, c &lt; 1. The character whose code is remainder
 * is inserted between the current character and next_char; then the current
 * character is deleted if b=0, and next_char is deleted if c=0; then we pass
 * over a characters to reach the next current character (which may have a
 ...
 * </p>
 * <p>
 * If the very first instruction of the lig_kern array has skip_byte=255, the
 * next_char byte is the so-called right boundary character of this font; the
 * value of next_char need not lie between bc and ec. If the very last
 ...
 * 256op_byte+remainder. The interpretation is that TeX puts implicit boundary
 * characters before and after each consecutive string of characters from the
 * same font. These implicit characters do not appear in the output, but they
 * can affect ligatures and kerning.
 * </p>
 * <p>
...
public class TfmLigKernArray implements PlFormat, Serializable {
            ...
            if (lkc.getActivity() != TfmLigKernCommand.PASSTHROUGH) {
                if (!lkc.meansRestart()) {
                    ...
                    ligKernTable[currIns++] =
                            (lkc.meansKern()) ? makeKern(lkc, skip) : makeLig(
                                lkc, skip);
                }
            }
        }
     ...
     * @return Returns the skip amount of the next instruction in the final
     *         version of lig/kern program.
     */
     ...
     * a restart instructions and checks for validity. TFtoPL[67]
     * 
     ...
     * lig/kern table and checks for errors. TFtoPL[69]
     */
     ...
     * {@inheritDoc}
     * 
     * @see org.extex.font.format.pl.PlFormat#toPL(org.extex.font.format.pl.PlWriter)
     */
